...
class CustomCalendarState extends State<CustomCalendar> with TickerProviderStateMixin {
  ...
  DateTime displayDate = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
  ...
  Animatable<Color?>? _arrowColorTween = ColorTween(begin: Color(0x00FFA68A), end: Color(0xffFFA68A));

  /// Color tween for expand_less icon
  Animatable<Color?>? _monthColorTween = ColorTween(begin: Color(0xffEC520B), end: Color(0x00EC520B));
  ...
  void initState() {
    ...
    calList = [Column(mainAxisAlignment: MainAxisAlignment.spaceEvenly, mainAxisSize: MainAxisSize.min, children: rowListReturned)];
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    double calendarWidth = MediaQuery.of(context).size.width * 0.95;
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                height: 50,
                child: Padding(
                  padding: EdgeInsets.only(top: 13 * scaleFactor, bottom: 8 * scaleFactor, left: 16 * scaleFactor, right: 16 * scaleFactor),
                  child: Row(
                              ...
                              setState(() {
                                ...
                                calList = [
                                  Column(
                                      ...
                                      children: returnRowList(DateTime(showDate!.year, showDate!.month - 1, 1))),
                                  Column(
                                      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                      mainAxisSize: MainAxisSize.min,
              ...
              AnimatedBuilder(
                ...
                builder: (BuildContext context, Widget? child) => Container(
                  ...
                  child: Align(
                    ...
                    heightFactor: _anim!.value * (1 - collapsedHeightFactor!) + collapsedHeightFactor!,
                    child: Container(
                      width: calendarWidth,
  ...
  int getActiveRow() {
    ...
    for (int i = 0; i < rowValueList.length; i++) {
      for (int j = i; j < rowValueList[i].length; j++) {
        if (displayDate.month == DateTime.now().month && rowValueList[i].contains(DateTime.now().day) && monthChecks(i, rowValueList[i][j])) {
          activeRow = i + 1;
        }
      }
    }
    ...
    for (int i = 0; i < rowValueList.length; i++) {
      ...
      for (int j = 0; j < rowValueList[i].length; j++) {
        itemList.add(Expanded(
          child: InkWell(
            onTap: () {
              print('ay nooo ${rowValueList[i][j].toString()}');
            },
            child: Container(
                height: 22,
                width: 22,
                ...
                child: Center(
                  child: Text(
                    ...
                    style: (rowValueList[i][j] == DateTime.now().day && start.month == DateTime.now().month && start.year == DateTime.now().year) &&
                            !((i == 0 && rowValueList[i][j] > 7) || (i >= 4 && rowValueList[i][j] < 7))
                        ? TextStyle(
                            fontWeight: FontWeight.bold,
                          )
                        ...
                        : TextStyle(
                            ...
                            color: ((i == 0 && rowValueList[i][j] > 7) || (i >= 4 && rowValueList[i][j] < 7)) ? Colors.grey : Colors.black),
