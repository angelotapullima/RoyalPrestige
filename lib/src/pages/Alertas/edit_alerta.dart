...
class EditAlerta extends StatefulWidget {
  const EditAlerta({Key? key, required this.alerta}) : super(key: key);
  final AlertModel alerta;

  @override
  State<EditAlerta> createState() => _EditAlertaState();
}

class _EditAlertaState extends State<EditAlerta> {
  ...
  @override
  void initState() {
    _tituloController.text = widget.alerta.alertTitle.toString();
    _detalleController.text = widget.alerta.alertDetail.toString();
    fechaDato = widget.alerta.alertDate2.toString();
    horaDato = widget.alerta.alertHour2.toString();

    if (widget.alerta.idClient != '0') {
      final provider = Provider.of<BusquedaClienteController>(context, listen: false);
      provider.changeCliente(widget.alerta.idClient.toString(), widget.alerta.nombreCLiente.toString());
    }
                                ...
                                if (_detalleController.text.isNotEmpty) {
                                  if (fechaDato != 'Seleccionar') {
                                    if (horaDato != 'Seleccionar') {
                                      ...
                                      alertModel.idAlert = widget.alerta.idAlert;
                                      ...
                                      final res = await alertApi.editAlert(alertModel);
                                      ...
                                      if (res) {
                                        ...
                                        Navigator.pop(context);
                                        showToast2('alerta editada correctamente', Colors.green);
                                      } else {
                                        _cargando.value = false;
